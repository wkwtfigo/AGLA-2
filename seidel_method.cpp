...
public:
    ...
    Matrix operator-(const Matrix& other) const override {
        if (dynamic_cast<const SquareMatrix*>(&other)) {
            const auto& squareMatrix = static_cast<const SquareMatrix&>(other);
            Matrix result(n, m);
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < m; ++j) {
                    result.setValue(i, j, getValue(i, j) - squareMatrix.getValue(i, j));
                }
            }
            return result;
        } else {
...
void jacobiMethod(SquareMatrix A, ColumnVector b, double eps) {
    if (!A.diagonalPredominance()) {
        cout << "The method is not applicable" << endl;
        return;
    }

    SquareMatrix alpha = A;
    ...
    for (int i = 0; i < A.getN(); i++) {
        ...
        beta.setValue(i, 0, b.getValue(i, 0) / first);
    }
    cout << "beta:" << endl;
    cout << beta;

    SquareMatrix B = SquareMatrix(A.getN());
    for (int i = 0; i < B.getN(); i++) {
        for (int j = 0; j < B.getM(); j++) {
            if (i > j) {
                B.setValue(i, j, alpha.getValue(i, j));
            } else {
                B.setValue(i, j, 0);
            }
        }
    ...
    SquareMatrix C = SquareMatrix(A.getN());
    for (int i = 0; i < C.getN(); i++) {
        for (int j = 0; j < C.getM(); j++) {
            if (i < j) {
                C.setValue(i, j, alpha.getValue(i, j));
            } else {
                C.setValue(i, j, 0);
            }
        }
    ...
    cout << "I-B:" << endl;
    Identity I = Identity(B.getN());
    Matrix L = I - B;
    cout << L;

    cout << "(I-B)_-1:" << endl;
    Matrix K = inverseWithoutPrint(L);
    cout << K;
